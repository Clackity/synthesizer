...
namespace nmain {
	bool init() {
		...
		//init_intel();
		nrandom::init();
		//numthreads = numthreads ? numthreads : ippGetNumCoresOnDie();
		numthreads = 4; // TODO: detect number of virtual cores available, use them all
		
		// this next line is to try to get multiprocessing to work on a particular AMD chip
